...
export default function CouponsPage() {
  ...
  const load = () => adminFetch('/api/coupons').then(d => setCoupons(d.data.coupons)).catch(console.error)
  useEffect(() => { load() }, [])
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        {coupons.length === 0 ? <div className="p-12 text-center text-gray-400 text-sm">No coupons yet.</div> : (
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 text-left">
              <th className="px-6 py-3 text-xs font-medium text-gray-500">Code</th>
              <th className="px-6 py-3 text-xs font-medium text-gray-500">Discount</th>
              <th className="px-6 py-3 text-xs font-medium text-gray-500">Used</th>
              <th className="px-6 py-3 text-xs font-medium text-gray-500">Expires</th>
            ...
            <tbody>
              {coupons.map(c => (
                <tr key={c.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3">{c.discountType === 'percentage' ? `${c.discountValue}%` : `LKR ${c.discountValue}`}</td>
                  <td className="px-6 py-3">{c.usedCount}{c.maxUses ? ` / ${c.maxUses}` : ''}</td>
                  <td className="px-6 py-3 text-gray-500 text-xs">{c.expiresAt ? formatDate(c.expiresAt) : '—'}</td>
                  <td className="px-6 py-3"><span className={`px-2 py-0.5 rounded text-xs font-medium ${c.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{c.active ? 'Active' : 'Inactive'}</span></td>
